...
public class StmtCalculatorExpr extends Visitor<Void>{
    ...
    public Void visit(FuncDefExtDec funcDefExtDec) {
        ...
        System.out.println("Line "+ funcDefExtDec.getLine() +": Stmt function " + tempFuncNames + " = " + numStmt + " " + funcDefExtDec.getNumOfArgs());
...
//            System.out.println("Line "+ unaryExpr.getLine() +"Expr " + unaryExpr.getOperator().toString());
    ...
    public Void visit(ArrayDeclarator arrayDeclarator) {
        if(arrayDeclarator.getBaseDeclarator() != null){
            if(arrayDeclarator.getBaseDeclarator() instanceof IdentifierDeclarator){
                String tempIdentifier = ((IdentifierDeclarator) arrayDeclarator.getBaseDeclarator()).getName();
                int tempLine = ((IdentifierDeclarator) arrayDeclarator.getBaseDeclarator()).getLine();
//                System.out.println("Line " + tempLine + ": Expr " + tempIdentifier);
            }
        }
        return null;
    }
...
//            printExpr(iterationStatement.getCondition(), 0);
        if(iterationStatement.getForCondition() != null) {
            tempForLine = iterationStatement.getLine();
//            System.out.println("Line " + iterationStatement.getLine() + ": Expr =");
    ...
    public void printExpr(Expr expr, int state)
    {
        ...
        if(expr instanceof AssignmentExpr){
            ...
            if(state == 8)
                ...
                System.out.println("Line " + tempForLine + ": Expr " + ((UnaryExpr) expr).getOperator());
            else
